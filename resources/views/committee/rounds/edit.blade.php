    ...
    <div class="card" style="max-width: 600px;">
        <form action="{{ route('committee.rounds.update', [$competition, $round]) }}" method="POST">
            ...
            <div class="form-group mb-4">
                ...
                <select name="scoring_type_id" id="scoring_type_id" class="form-input w-full" required>
                    ...
                    @foreach($scoringTypes as $st)
                        <option value="{{ $st->id }}" {{ old('scoring_type_id', $round->scoring_type_id) == $st->id ? 'selected' : '' }}>
                            {{ $st->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group mb-6 flex items-center gap-2">
                <input type="checkbox" name="is_bracket" id="is_bracket" value="1" {{ old('is_bracket', $round->is_bracket) ? 'checked' : '' }} class="rounded border-2 border-black text-primary focus:ring-0 w-5 h-5 cursor-pointer">
