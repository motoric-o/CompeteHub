...
<div class="section animate-in">
    ...
    <div class="grid grid-cols-3">
        @foreach($availableCompetitions as $comp)
            <div class="card" id="competition-card-{{ $comp->id }}">
                <h3 class="card-title">{{ $comp->name }}</h3>
                <p class="text-muted" style="font-size: 0.85rem; margin-bottom: 0.5rem;">
                    {{ Str::limit($comp->description, 80) }}
                </p>
                <div class="flex gap-1 items-center" style="font-size: 0.8rem; color: var(--color-text-dim); margin-bottom: 1rem;">
                    <span>💰 Rp {{ number_format($comp->registration_fee, 0, ',', '.') }}</span>
                    <span>•</span>
                    <span>👥 Kuota: {{ $comp->quota ?? '∞' }}</span>
                </div>
                <a href="{{ route('teams.create', ['competition_id' => $comp->id]) }}" class="btn btn-primary btn-sm">
                    Buat Tim →
                </a>
            </div>
        @endforeach
